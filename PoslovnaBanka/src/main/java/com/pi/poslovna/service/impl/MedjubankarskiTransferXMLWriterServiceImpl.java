...
public class MedjubankarskiTransferXMLWriterServiceImpl implements MedjubankarskiTransferXMLWriterService {
	...
	public void createClearingXML(InterbankTransfer it) {
		try {
			...
			Document doc = docBuilder.newDocument();
			Element rootElement = doc.createElement("kliring_nalog");
			doc.appendChild(rootElement);
			...
			String rootPath = System.getProperty("user.dir");
			String filePath = rootPath + "/src/main/resources/kliring/" + "kliring"+ it.getId().toString() +".xml";
			TransformerFactory transformerFactory = TransformerFactory.newInstance();
			Transformer transformer = transformerFactory.newTransformer();
			transformer.setOutputProperty(OutputKeys.INDENT, "yes");
			transformer.setOutputProperty("{http://xml.apache.org/xslt}indent-amount", "2");
			...
			System.out.println("Kliring nalog se snimio kao [ " + filePath + " ]");
		} catch(Exception e) {
			e.printStackTrace();
		}
